...
export function StockHeader({ stockInfo, stocks, selectedStockId, onSelectStock, contentWidthRef }: StockHeaderProps) {
  ...
  const filtered = stocks
    ...
    .filter((s) =>
      category === "관심" ? favorites.includes(s.id)
      : category === "상승" ? s.changeRate > 0
      : category === "하락" ? s.changeRate < 0
      : true
    )
    .sort((a, b) => {
  ...
  return (
    <div ref={containerRef} className="w-full bg-[#181a20] py-1 rounded-xl">
      <div
        className="relative"
      ...
      >
        {stockInfo ? (
          ...
          <>
            ...
            <svg className="ml-auto w-4 h-4 text-zinc-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
            </svg>
          </>
        )}
      ...
      {/* 드롭다운 */}
      <div
        className={`absolute top-full left-0 right-0 mt-1.5 z-50 grid transition-[grid-template-rows] duration-200 ease-out ${
          ...
          <div className="max-h-108 overflow-y-auto">
            {filtered.length > 0 ? filtered.map((stock) => {
                ...
                >
                  <button
                    ...
                    className={`shrink-0 p-1 transition-colors ${
                      isFav ? "text-[#F59E0B]" : "text-zinc-700 hover:text-zinc-500"
                    }`}
                  >
                    <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill={isFav ? "currentColor" : "none"} stroke="currentColor" strokeWidth={2}>
                      <path strokeLinejoin="round" d="M12 3l2.9 5.9 6.5.9-4.7 4.6 1.1 6.5-5.8-3-5.8 3 1.1-6.5L2.6 9.8l6.5-.9L12 3z" />
                    </svg>
                  </button>

                  <button
                    onClick={() => { onSelectStock(stock.id); closeDropdown(); }}
                  ...
                  >
                    ...
                    <span className={`text-xs font-semibold text-right w-16 tabular-nums ${perColor}`}>
                      {isUp ? "+" : ""}{per.toFixed(2)}%
                    </span>
                  </button>
                </div>
              ...
              <div className="pt-16 pb-10 text-center text-zinc-600 text-xs">
                {category === "관심" && !query ? "관심 종목이 없습니다" : "검색 결과 없음"}
              </div>
            )}
          </div>
        </div>
